describe('signup form tests', () => {
  ...
  it('should signup with valid credentials', () => {
    ...
    cy.get('@emailInput').should('have.attr', 'type', 'email').type('kansy841@gmail.com');

    cy.getDataTest('signup-phone').as('phoneNumberInput');
    cy.get('@phoneNumberInput').should('have.attr', 'type', 'tel').type('0790519306');
    ...
    cy.get('@passwordInput').should('have.attr', 'type', 'password').type('pass123');

    cy.getDataTest('signup-confirmpassword').as('confirmPasswordInput');
    cy.get('@confirmPasswordInput').should('have.attr', 'type', 'password').type('pass123');
  ...
  it('should show validation errors for empty fields', () => {
    cy.getDataTest('signup-submitbtn').as('submitButton');
    cy.get('@submitButton').should('contain.text', 'Create Account').click();
