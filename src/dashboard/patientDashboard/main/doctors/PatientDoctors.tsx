...
import { Search, Stethoscope, Star, Calendar, Phone, Mail, XCircle, MapPin, ChevronRight } from 'lucide-react';
...
const appointmentSchema = yup.object({
  ...
  appointmentDate: yup.string().required('Date is required').test(
    'future-date', 
...
const PatientDoctors = () => {
  ...
  const { data: specData } = doctorsAPI.useGetDoctorBySpecializationQuery(selectedSpecialization, { skip: !selectedSpecialization });
  const [createAppointment, { isLoading: isBookingLoading }] = appointmentsAPI.useCreateAppointmentMutation();

  const { register, handleSubmit, reset, setValue, formState: { errors } } = useForm<CreateAppointmentInputs>({
  ...
  const specializations = doctorsData?.data ? [...new Set(doctorsData.data.map(d => d.doctor.specialization))] : [];
  ...
  if (isLoading) return <div className="flex justify-center p-20"><span className="loading loading-spinner text-teal-600 w-12"></span></div>;
...
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
  {filteredDoctors().map((doc) => (
    <div key={doc.doctor.doctorId} className="bg-white rounded-[2rem] border border-gray-100 p-2 shadow-sm hover:shadow-xl transition-all group">
      ...
      <div className="px-6 pb-6 text-center">
        ...
        <div className="space-y-2 mb-4 text-left border-t border-gray-50 pt-4">
          ...
          <div className="mt-3">
  <p className="text-[10px] font-bold text-gray-400 uppercase mb-2 text-left">Available Days</p>
  ...
  <div className="flex flex-nowrap gap-2 overflow-x-auto pb-2 scrollbar-hide">
    {doc.doctor.availableDays?.map((day) => (
      <span 
        key={day} 
        className="inline-block whitespace-nowrap px-4 py-1.5 bg-teal-50 text-teal-700 rounded-full text-[10px] font-bold border border-teal-100 flex-shrink-0"
      >
        {day}
      </span>
    ))}
        ...
        <div className="grid grid-cols-2 gap-2 mb-6">
          <div className="bg-gray-50 rounded-2xl p-3 border border-gray-100">
            ...
            <span className="text-md font-black text-gray-900">{doc.doctor.experience || 0} Yrs</span>
          </div>
          <div className="bg-gray-50 rounded-2xl p-3 border border-gray-100">
            <span className="block text-xs font-bold text-gray-400 uppercase">Patients</span>
            <span className="text-md font-black text-gray-900">{doc.doctor.patients || 0}</span>
          </div>
        </div>
      ...
      <dialog id="booking_modal" className="modal backdrop:backdrop-blur-sm">
        <div className="modal-box p-0 rounded-[2.5rem] bg-white overflow-hidden max-w-2xl">
          {/* Header Gradient */}
          <div className="bg-gradient-to-r from-teal-500 to-pink-500 p-8 text-white">
            <h3 className="text-2xl font-black">Book New Appointment</h3>
            <p className="text-teal-50/80 font-medium">Schedule your consultation with our doctors</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6">
            <div className="space-y-6">
              <div>
                ...
                <select className="w-full bg-gray-50 p-4 rounded-2xl border border-gray-200 outline-none focus:ring-2 focus:ring-teal-500 font-bold" disabled>
                   <option>Dr. {doctorToBook?.user.firstName} {doctorToBook?.user.lastName} - {doctorToBook?.doctor.specialization}</option>
                </select>
              </div>
              ...
              {doctorToBook && (
                <div className="bg-teal-50/50 border border-teal-100 p-5 rounded-2xl flex items-center gap-4">
                  <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-white shadow-sm">
                    <img src={doctorToBook.user.image_url || 'https://via.placeholder.com/150'} className="w-full h-full object-cover" alt="Doctor" />
                  </div>
                  <div>
                    <h4 className="font-black text-gray-900">Dr. {doctorToBook.user.firstName} {doctorToBook.user.lastName}</h4>
                    <p className="text-teal-600 text-xs font-bold">{doctorToBook.doctor.specialization}</p>
                    <p className="text-gray-500 text-xs mt-1">Available: {doctorToBook.doctor.availableDays?.join(', ')}</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-black text-gray-400 uppercase ml-1">Appointment Date</label>
                  <input type="date" {...register('appointmentDate')} className="w-full bg-gray-50 p-4 rounded-2xl border border-gray-200 outline-none focus:ring-2 focus:ring-teal-500 font-bold" />
                  {errors.appointmentDate && <p className="text-red-500 text-[10px] font-bold ml-1 uppercase">{errors.appointmentDate.message}</p>}
                </div>

                <div className="flex flex-col gap-2">
                  <label className="text-xs font-black text-gray-400 uppercase ml-1">Time Slot</label>
                  <select {...register('timeSlot')} className="w-full bg-gray-50 p-4 rounded-2xl border border-gray-200 outline-none focus:ring-2 focus:ring-teal-500 font-bold appearance-none">
                    <option value="">Select Time</option>
                    {timeSlots.map(t => <option key={t} value={t}>{t.substring(0, 5)}</option>)}
                  </select>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-6 rounded-3xl flex items-center justify-between border border-gray-100">
              <div>
                ...
                <p className="text-2xl font-black text-teal-600">KSh {parseFloat(DEFAULT_FEE).toLocaleString()}</p>
                <p className="text-[10px] text-gray-400 font-bold">Payment due at appointment</p>
              </div>
              <div className="flex gap-3">
                <button type="button" onClick={handleCloseModal} className="px-6 py-4 font-bold text-gray-500 hover:text-gray-900 transition-colors">Cancel</button>
                <button 
                  type="submit" 
