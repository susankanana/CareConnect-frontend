...
const ChangeStatus = ({ appointment, refetch }: ChangeStatusProps) => {
  const [updateAppointmentStatus, { isLoading }] =
    appointmentsAPI.useUpdateAppointmentStatusMutation({
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <div className="bg-gray-50 rounded-lg p-4 mb-4">
            <p className="text-sm text-gray-700">
              ...
              <span
                className={`ml-2 font-semibold ${getStatusColor(appointment?.status || 'Pending')}`}
              >
                {appointment?.status}
              </span>
            </p>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            ...
            <ul className="text-xs text-blue-700 mt-1 space-y-1">
              ...
              <li>
                • <strong>Confirmed:</strong> Appointment is confirmed
              </li>
              <li>
                • <strong>Cancelled:</strong> Appointment is cancelled
              </li>
            </ul>
          </div>
          ...
          <div className="modal-action flex flex-col sm:flex-row gap-2">
            <button
              data-test="status-submit-button"
              type="submit"
