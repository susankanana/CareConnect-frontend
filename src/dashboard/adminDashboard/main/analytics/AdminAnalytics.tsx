...
const AdminAnalytics = () => {
  ...
  const processedData = React.useMemo(() => {
    ...
    const doctorPerformance = doctorsData.data
      .map((doc) => ({
        ...
        rev: appointments
          .filter((a) => a.doctor?.id === doc.user.userId)
          .reduce((s, a) => s + parseFloat(a.totalAmount), 0),
      }))
  ...
  const StatCard = ({ title, value, growth, icon: Icon, isCurrency = false }: any) => (
    <div className="bg-white rounded-4xl p-8 border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-teal-500/5 transition-all group">
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-[3rem] p-10 border border-gray-100 shadow-sm">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-xl font-black italic text-gray-900 lowercase">
              revenue trajectory
            </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* Top Doctors */}
        <div className="bg-white rounded-[3rem] p-10 border border-gray-100 shadow-sm">
          <h3 className="text-xl font-black italic mb-8 lowercase text-teal-600">
            elite performers
          </h3>
        ...
        <div className="bg-linear-to-br from-[#004d4d] to-[#006666] rounded-[3rem] p-10 text-white flex flex-col justify-between shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -mr-32 -mt-32 blur-3xl" />
          <div className="relative z-10">
            <h3 className="text-2xl font-black italic mb-2 lowercase tracking-tight">
              intelligence tools
            </h3>
            <p className="text-teal-100/60 text-sm font-medium mb-8">
              generate and export detailed system audits
            </p>
            ...
            <div className="space-y-3">
              ...
              <button className="w-full bg-pink-500 hover:bg-pink-600 p-4 rounded-2xl flex items-center justify-between group transition-all">
                <span className="text-sm font-bold lowercase italic text-white">
