...
} from '../../../../reducers/complaints/complaintsAPI';
...
const schema = yup.object({
  status: yup
    .string()
    .oneOf(['Open', 'In Progress', 'Resolved', 'Closed'])
    ...
    <dialog id="change_status_modal" className="modal sm:modal-middle">
      <div className="modal-box bg-white w-full max-w-xs sm:max-w-lg mx-auto rounded-lg border border-gray-200">
        <div className="bg-gradient-to-r from-teal-500 to-pink-500 -m-6 mb-6 p-6 rounded-t-lg">
          ...
          <p className="text-teal-100 text-sm mt-1">Subject: {complaint?.subject}</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            ...
            <ul className="text-xs text-blue-700 mt-1 space-y-1">
              <li>
                • <strong>Open:</strong> New complaint, needs attention
              </li>
              <li>
                • <strong>In Progress:</strong> Being actively worked on
              </li>
              <li>
                • <strong>Resolved:</strong> Issue has been fixed
              </li>
              <li>
                • <strong>Closed:</strong> Complaint is finalized
              </li>
            </ul>
          </div>
          ...
          <div className="modal-action flex flex-col sm:flex-row gap-2">
            <button
              data-test="submit-status-change"
              type="submit"
              className="btn bg-teal-600 hover:bg-teal-700 text-white border-none w-full sm:w-auto"
              disabled={isLoading}
            >
              {isLoading ? (
                <>
